...
(function () {
  ...
  var FIRST_BUILD_KEY = demoMode ? 'erosiris.auroraFirstBuildV1' : 'signalpath.firstBuildV1';
  var WELCOME_MS = 3000;
  var WELCOME_LINES = [
    '来挑战一下：输入 9 空格 3，再按回车！',
    '第一次来？点我，我会一步一步带你玩转工作台。',
    '只要两个数量，就能超级快地搭好第一套系统。',
    '不用怕点错，清空和撤销都在，我陪你试一遍。',
    '不会导入模板？交给小蝶带路就好啦。',
    '想把工程备份得稳稳的？我可以教你。',
    '每个按钮我都认识，点我一起逛逛吧！'
  ];

  function loadFirstBuild() {
    try {
      var data = JSON.parse(localStorage.getItem(FIRST_BUILD_KEY) || 'null');
      return data && typeof data === 'object' ? data : { status: 'new' };
    } catch (e) { return { status: 'new' }; }
  }
  var firstBuild = loadFirstBuild();
  function saveFirstBuild(status) {
    firstBuild.status = status;
    firstBuild.updatedAt = Date.now();
    try { localStorage.setItem(FIRST_BUILD_KEY, JSON.stringify(firstBuild)); } catch (e) {}
  }
  function firstBuildAvailable() {
    return firstBuild.status !== 'completed' && (Store.state.devices || []).length === 0;
  }
  var ICON_SRCS = [
  ...
  var COURSES = {
    ...
    'advanced-global': {
      ...
      steps: [
        ...
        { text: '顶栏「清设备」只会清除当前工程的设备与连线，案例模板、库存数量和预设都会保留；执行后也可以立即撤销。教学不会替你点击。',
          target: '#btn-clear', callout: '清设备 · 保留案例库存', manual: true, ensure: 'main',
          extra: '<div class="gd-course-warn">小蝶只讲解，不会执行清设备操作。模板与库存不会被删除。</div>' },
    ...
    'advanced-views': {
      ...
      steps: [
        ...
        { text: '「本场阵容」整理当前演出的乐手、输入需求和文字站位。',
          target: '[data-view="show"]', callout: '当前演出阵容', manual: true, ensure: 'main' },
        { text: '「预备场」单独管理多个候场阵容、入场时间与批量人员操作。',
          target: '[data-view="standby"]', callout: '候场安排', manual: true, ensure: 'main' },
        { text: '「舞台建模」承接 Windows Stage Studio 导出的场地与舞台预览。',
          target: '[data-view="stage"]', callout: '本地建模结果', manual: true, ensure: 'main' },
    ...
    'advanced-canvas': {
      ...
      steps: [
        ...
        { text: '「清设备」清除当前连线图里的设备及关联连线，但不会删除模板库。教学不会替你点击。',
          target: '#btn-clear-devices-diagram', callout: '清除画布设备 · 保留库存', manual: true, ensure: 'wiring',
          extra: '<div class="gd-concept">只清当前画布设备与连线，案例模板、库存和预设都会保留。</div>' },
  ...
  STEPS.home = function () {
    var s = appState();
    var needsFirstBuild = firstBuildAvailable();
    ...
    var text = demoNeedsCase
      ...
      : needsFirstBuild
      ? '嗨，我是小蝶。第一次不用先学会所有按钮，我们只填全频和超低两个数量，就能超级快地完成第一套系统。'
      : courseRun
    ...
    var demoCaseActions = demoNeedsCase
      ...
      : '';
    if (needsFirstBuild) chips.push(chip('first-build-start', '搭建我的第一套系统', 'primary'));
  ...
  STEPS.counts = function () {
    ...
    function opts(list) {
      var html = '<option value="">智能推荐（库存优先）</option>';
    ...
    return {
      id: 'counts',
      text: '超级快，我们先只填两个数量：你要几只全频、几只超低？型号可以交给我智能推荐，功放、DSP 和调音台我来算。' +
        (s.hasUserTpls ? '' : '（现在用的是内置模板，导入你的型号表会更准）'),
      chips: [
        chip('counts-apply', '帮我超级快地配好', 'primary'),
        chip('import', '📥 先导入型号'),
      ],
      extra: extra
    };
  ...
  STEPS.afterCounts = function () {
    return {
      id: 'afterCounts',
      text: '很好，最关键的一步已经完成！功放、DSP 和调音台都已自动计算。看一眼卡片，下一步确认音响组。',
      chips: [
  ...
  STEPS.create = function () {
    return {
      id: 'create',
      text: '只差最后一步：创建系统。我可以直接帮你点，也可以把按钮点亮让你亲手完成。',
      chips: [
  ...
  STEPS.firstBuildIntro = function () {
    return {
      id: 'firstBuildIntro',
  ...
  STEPS.firstBuildReview = function () {
    return {
      id: 'firstBuildReview',
      text: '欢迎页的数量已经带进来了，太方便了！系统正在实时反推设备规格。检查一下推荐型号，然后确认音响组就可以创建。',
      chips: [
        chip('first-build-confirm', '型号没问题，继续', 'primary'),
        chip('counts', '重新填写数量')
      ],
      extra: '<div class="gd-course-done">第一步已完成：数量已经成功带入。</div>'
    };
  };

  STEPS.firstBuildSuccess = function () {
    return {
      id: 'firstBuildSuccess',
      text: '完成！你的第一套系统已经建立好了。系统按当前模板与库存智能选择型号，调音台、DSP、功放、9 只全频、3 只超低和连线已一次生成。做得很棒！',
      chips: [
        chip('first-build-clear-help', '认识清空重来', 'primary'),
        chip('first-build-import', '导入我的模板'),
        chip('first-build-view', '先看看我的系统')
      ],
      extra: '<div class="gd-course-done"><b>第一次搭建成功</b> · 点错可以撤销，想重做也不会丢失模板和库存。</div>'
    };
  };

  STEPS.firstBuildClear = function () {
    return {
      id: 'firstBuildClear',
      text: '想重新来一遍时，点“清设备”就好。它只清除当前画布里的设备和连线，模板库、库存和预设都会保留。清空后小蝶会带你再来一次，这次由你亲自选择指定音响型号。',
      chips: [
        chip('first-build-specific', '我已清空，选择指定型号', 'primary'),
        chip('first-build-import', '先学导入模板'),
        chip('first-build-finish', '我已经会了')
      ],
      extra: '<div class="gd-concept">放心试：清设备不等于删除资料，模板与库存仍然安全地留在库里。</div>'
    };
  };

  STEPS.firstBuildSpecific = function () {
    var step = STEPS.counts();
    step.id = 'firstBuildSpecific';
    step.text = '很好，第二次你来决定型号。填写全频和超低数量，再点开右侧下拉表选择指定音响；这就是日常做不同项目时的真实用法。';
    step.chips = [
      chip('counts-apply', '用我选的型号反推', 'primary'),
      chip('first-build-import', '先导入我的型号')
    ];
    step.extra += '<div class="gd-course-done">这次不再固定 206M / K212S，你可以从模板库中自由选型。</div>';
    return step;
  };

  STEPS.firstBuildTemplatePreview = function () {
    return {
      id: 'firstBuildTemplatePreview',
      text: '模板库已经打开。先别急着导入，用半分钟看一眼现有模板表：每一行就是一个设备型号，功率、阻抗和输入输出都会用于后续反推。',
      chips: [
        chip('first-build-template-download', '我看过了，继续', 'primary'),
        chip('first-build-template-later', '稍后再学')
      ],
      extra: '<div class="gd-concept"><b>先看结构：</b>上方可切换全频、超低、有源、功放、DSP 和调音台；下方是已有型号与参数。</div>'
    };
  };

  STEPS.firstBuildTemplateDownload = function () {
    return {
      id: 'firstBuildTemplateDownload',
      text: '看懂模板表后，下一步就是下载官方填写表。点击我标亮的“下载模板总表”，它会包含全频、超低、有源音响、功放、DSP 和调音台。',
      chips: [
        chip('first-build-locate-download', '点亮下载按钮', 'primary'),
        chip('first-build-template-preview', '返回看模板表')
      ],
      extra: '<div class="gd-course-done">请亲自点击下载按钮；下载完成后小蝶会自动继续讲解。</div>'
    };
  };

  STEPS.firstBuildTemplateFill = function () {
    return {
      id: 'firstBuildTemplateFill',
      text: '下载好了，超级快！用 Excel 打开模板总表，每个工作表填对应设备。不要改表头、不要合并单元格，填完后保存原文件即可。',
      chips: [
        chip('first-build-template-import-ready', '我填好了，带我导入', 'primary'),
        chip('first-build-finish', '稍后再填')
      ],
      extra: '<div class="gd-fill-guide"><b>填写重点</b>' +
        '<span>型号名称：必填，例如 206M</span>' +
        '<span>功率：必填，用于功放反推</span>' +
        '<span>阻抗：无源音响必填，有源不填</span>' +
        '<span>输入 / 输出：按设备真实接口数填写</span></div>'
    };
  };

  STEPS.firstBuildTemplateImport = function () {
    return {
      id: 'firstBuildTemplateImport',
      text: '回到模板库后，选择“导出导入”，再点“导入CSV模板”选择刚填好的 .xls 或 .csv 文件。同名型号会自动更新，不会重复堆积。',
      chips: [
        chip('first-build-locate-import', '点亮导入按钮', 'primary'),
        chip('first-build-finish', '完成首次引导')
      ],
      extra: '<div class="gd-course-done">到这里，你已经完成“看模板 → 下载 → 填写 → 导入”的完整流程。</div>'
    };
  };

  STEPS.firstBuildDone = function () {
    return {
      id: 'firstBuildDone',
      text: '第一次系统搭建完成！以后你只要填数量，就能快速得到一整套可继续编辑的系统。小蝶一直在右下角，随时可以继续带路。',
      chips: [
        chip('first-build-view', '回到我的系统', 'primary'),
        chip('power-check', '继续做功率检查')
      ],
      extra: '<div class="gd-course-done">已完成首次引导 · 下次进入不会再自动打扰你。</div>'
    };
  };
  ...
  STEPS['faq-undo'] = function () {
    return {
      id: 'faq-undo',
      text: function () { return '几乎所有操作都能撤销：' + SP.actionCombo('undo') + ' 撤销、' +
        SP.actionCombo('redo') + ' 重做（顶栏也有按钮，我点亮给你看）。放心大胆点。'; },
  ...
  var ACTIONS = {
    ...
    'first-build-later': function () {
      saveFirstBuild('paused');
      closePanel();
    },
    'first-build-confirm': function () {
      runCommand('确认全频');
      runCommand('确认超低');
      go('create');
    },
    'first-build-clear-help': function () {
      go('firstBuildClear');
      setTimeout(function () {
        spotlight('#btn-clear-devices-diagram', { persist: true, text: '清空当前设备，模板和库存会保留' });
      }, 80);
    },
    'first-build-specific': function () {
      clearSpot();
      if (appState().devices) {
        go('firstBuildClear');
        setTimeout(function () {
          spotlight('#btn-clear-devices-diagram', { persist: true, text: '请先点这里清空当前系统' });
        }, 80);
        return;
      }
      go('firstBuildSpecific');
    },
    'first-build-import': function () {
      clearSpot();
      saveFirstBuild('template-guide');
      if (SP.openTemplatePanel) SP.openTemplatePanel();
      go('firstBuildTemplatePreview');
      setTimeout(function () {
        spotlight('.tpl-lib-list', { persist: true, text: '先看一眼现有型号和参数' });
      }, 120);
    },
    'first-build-template-preview': function () {
      clearSpot();
      var previewTab = resolveTarget('[data-tpl-mode="preview"]');
      if (previewTab && previewTab.click) previewTab.click();
      go('firstBuildTemplatePreview');
      setTimeout(function () {
        spotlight('.tpl-lib-list', { persist: true, text: '先看一眼现有型号和参数' });
      }, 80);
    },
    'first-build-template-download': function () {
      clearSpot();
      var downloadTab = resolveTarget('[data-tpl-mode="download"]');
      if (downloadTab && downloadTab.click) downloadTab.click();
      go('firstBuildTemplateDownload');
      setTimeout(function () {
        spotlight('#tplp-dl-all', { persist: true, text: '下载官方模板总表' });
      }, 80);
    },
    'first-build-locate-download': function () {
      spotlight('#tplp-dl-all', { persist: true, text: '点这里下载模板总表' });
    },
    'first-build-template-import-ready': function () {
      clearSpot();
      var importTab = resolveTarget('[data-tpl-mode="import"]');
      if (importTab && importTab.click) importTab.click();
      go('firstBuildTemplateImport');
      setTimeout(function () {
        spotlight('#tplp-csv-single', { persist: true, text: '选择填好的模板文件' });
      }, 80);
    },
    'first-build-locate-import': function () {
      spotlight('#tplp-csv-single', { persist: true, text: '导入填好的 .xls 或 .csv' });
    },
    'first-build-template-later': function () {
      clearSpot();
      saveFirstBuild('completed');
      closePanel();
      if (SP.closeModal) SP.closeModal();
    },
    'first-build-finish': function () {
      clearSpot();
      saveFirstBuild('completed');
      go('firstBuildDone');
    },
    'first-build-view': function () {
      clearSpot();
      saveFirstBuild('completed');
      closePanel();
      if (SP.switchView) SP.switchView('wiring');
      SP.toast('第一套系统已经准备好，可以继续查看和调整');
    },
    ...
    'do-create': function () {
      var b = el('ql-confirm');
      var firstBuildActive = firstBuild.status === 'started';
      if (b && b.click) b.click();
      if (!firstBuildActive) go('afterCreate');
    },
  ...
  function openPanel(step) {
    var host = el('guide-float');
    var welcome = el('guide-welcome');
    if (!host) return false;
    if (welcome && welcome.classList) welcome.classList.add('is-hidden');
    host.classList.add('is-open');
    go(step);
    return true;
  }

  function maybeStartFirstBuild(opt) {
    opt = opt || {};
    if (!opt.fromWelcome || ['templates', 'wiring', 'report'].indexOf(opt.action) >= 0) return false;
    if (!firstBuildAvailable() || firstBuild.status === 'paused') return false;
    saveFirstBuild('started');
    return openPanel(opt.command ? 'firstBuildReview' : 'firstBuildIntro');
  }

  function beginFirstBuildSilently() {
    if (!firstBuildAvailable()) return false;
    saveFirstBuild('started');
    return true;
  }

  function firstBuildCreated(added) {
    if (firstBuild.status !== 'started' || !added || !added.length) return false;
    saveFirstBuild('built');
    setTimeout(function () { openPanel('firstBuildSuccess'); }, 120);
    return true;
  }
  ...
  SP.Guide = {
    ...
    maybeStartFirstBuild: maybeStartFirstBuild,
    beginFirstBuildSilently: beginFirstBuildSilently,
    firstBuildCreated: firstBuildCreated,
    firstBuildStatus: function () { return firstBuild.status; },
  ...
  SP.initGuide = function () {
    ...
    document.addEventListener('click', function (e) {
      courseEvent('click', e.target);
      var downloaded = e.target && e.target.closest && e.target.closest('#tplp-dl-all');
      if (downloaded && current === 'firstBuildTemplateDownload') {
        clearSpot();
        setTimeout(function () { go('firstBuildTemplateFill'); }, 120);
      }
      var cleared = e.target && e.target.closest && e.target.closest('#btn-clear-devices-diagram');
      if (cleared && current === 'firstBuildClear') {
        clearSpot();
        setTimeout(function () { go('firstBuildSpecific'); }, 120);
      }
    });
