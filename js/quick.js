...
(function () {
  ...
  function afterCreate(added) {
    ...
    if (SP.Guide && SP.Guide.firstBuildCreated) SP.Guide.firstBuildCreated(added);
  }
  ...
  SP.openQuickLayout = function (opt) {
    ...
    function rvCardsHtml() {
      ...
      var adds = '<div class="rv-add-row rv-add-row-top" aria-label="添加音响组">' +
        ...
        '</div>';
      return adds + '<div class="rv-cards">' + cards + '</div>';
    }
    ...
    SP.openModal(
      ...
      '<span class="head-note">' + esc(SP.actionCombos('quickLayout').slice(0, 2).join(' / ')) + ' · 数字填格 · 空格下一格 · 退格清空/回上格 · Shift 展开有源</span>' +
    ...
    function applySelectionToGroup(g, sel) {
      ...
      if (sel.pick === 'rank') {
        ...
        var rankedItems = rvTemplateItemsFor(g);
        var stockItems = rankedItems.filter(function (it) { return it.stock !== null; });
        var recommended = stockItems[0] || rankedItems.filter(function (it) {
          var specs = (it.t && it.t.specs) || {};
          return it.power > 0 && (g.active || Number(specs.ohms) > 0);
        })[0];
        idx = recommended ? recommended.idx : -1;
    ...
    if (mode === 'reverse') {
      ...
      if (opt.autoCreate) {
        applySpeakerVoiceCommand('全频要206M，超低型号是K212S');
        applySpeakerVoiceCommand('确认全频');
        applySpeakerVoiceCommand('确认超低');
        confirm2();
      }
    } else {
      qlCalcShow();
