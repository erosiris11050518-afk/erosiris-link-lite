...
(function () {
  ...
  function renderList(host, multiCount) {
    ...
    if (!st.devices.length) {
      var quickKey = SP.actionCombo ? SP.actionCombo('quickLayout') : 'Alt+1';
      host.innerHTML = '<div class="empty-hint">还没有设备。<br>点上方「＋ 添加」，' +
        '或按 <kbd>' + esc(quickKey) + '</kbd> 快速布局。</div>';
      return;
    }
  ...
  function renderDetail(host, dev) {
    ...
    host.innerHTML =
      ...
      '<button class="btn ghost sm" id="insp-clear-in" title="清空进入本设备的连线（' + esc(SP.actionCombo('clearIn')) + '）">清 IN</button>' +
      '<button class="btn ghost sm" id="insp-clear-out" title="清空从本设备出去的连线（' + esc(SP.actionCombo('clearOut')) + '）">清 OUT</button>' +
      '<button class="btn ghost sm" id="insp-copy" title="复制本设备并自动编号（' + esc(SP.actionCombo('duplicate')) + '）">⧉ 复制</button>' +
    ...
    menu.innerHTML =
      ...
      (dev.type === 'mixer'
        ...
        ? '<div class="ctx-actions">' +
          ...
          '<button class="mini-act" data-ctx-clear-in>清 IN ' + esc(SP.actionCombo('clearIn')) + '</button>' +
          '<button class="mini-act" data-ctx-clear-out>清 OUT ' + esc(SP.actionCombo('clearOut')) + '</button>' +
          '<button class="mini-act" data-ctx-copy>复制 ' + esc(SP.actionCombo('duplicate')) + '</button>' +
  ...
  SP.promptTemplateLibImport = function (data, refreshFn) {
    ...
    openModal(
      ...
      '· <b>覆盖</b>：清空当前模板库后完全替换为该文件内容（可 ' + esc(SP.actionCombo('undo')) + ' 撤销）。</div>' +
      '</div>' +
      '<div class="modal-foot">' +
