...
(function () {
  ...
  SP.toast = function (msg, warn) {
    ...
    if (SP.localizeShortcutText) msg = SP.localizeShortcutText(msg);
    var t = el('sp-toast');
    if (!t) {
  ...
  function exportConfig() {
    ...
    (data.devices || []).forEach(function (dv) {
      [dv.imgId, dv.panelImgId].forEach(addImage);
    });
    function addPerformerImages(p) {
      addImage(p.photoId);
      (p.photoGallery || []).forEach(function (photo) { addImage(photo.imageId); });
    }
    (data.performerTemplates || []).forEach(addPerformerImages);
    (data.showRoster || []).forEach(addPerformerImages);
    (data.standbyRosters || []).forEach(function (roster) { (roster.members || []).forEach(addPerformerImages); });
    if (data.showPlan) addImage(data.showPlan.previewImgId);
    if (data.mixerPlanning) {
      addImage(data.mixerPlanning.inputPanelImgId);
      addImage(data.mixerPlanning.outputPanelImgId);
    }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    SP.switchView = function (name) {
      ...
      document.querySelectorAll('[data-mobile-view]').forEach(function (x) {
        x.classList.toggle('active', x.dataset.mobileView === name);
      });
      var mobileMore = el('lite-mobile-more');
      var mobileMoreBtn = document.querySelector('[data-mobile-more]');
      if (mobileMore) mobileMore.hidden = true;
      if (mobileMoreBtn) {
        mobileMoreBtn.setAttribute('aria-expanded', 'false');
        mobileMoreBtn.classList.toggle('active', ['mixer', 'performers', 'show', 'console', 'standby', 'stage'].indexOf(name) >= 0);
      }
      ...
      if (name === 'performers' && SP.renderPerformers) SP.renderPerformers();
      if (name === 'show' && SP.renderShowWorkflow) SP.renderShowWorkflow();
      if (name === 'console' && SP.renderConsolePlanning) SP.renderConsolePlanning();
      if (name === 'standby' && SP.renderStandbyWorkflow) SP.renderStandbyWorkflow();
      if (name === 'stage' && SP.renderStageWorkflow) SP.renderStageWorkflow();
    };
    el('main-tabs').querySelectorAll('.tab').forEach(function (t) {
      t.addEventListener('click', function () { SP.switchView(t.dataset.view); });
    });
    document.querySelectorAll('[data-mobile-view]').forEach(function (t) {
      t.addEventListener('click', function () { SP.switchView(t.dataset.mobileView); });
    });
    var mobileMorePanel = el('lite-mobile-more');
    var mobileMoreToggle = document.querySelector('[data-mobile-more]');
    ...
    if (mobileMoreToggle && mobileMorePanel) {
      mobileMoreToggle.addEventListener('click', function (e) {
        ...
        mobileMoreToggle.setAttribute('aria-expanded', mobileMorePanel.hidden ? 'false' : 'true');
      });
      var mobileMoreClose = mobileMorePanel.querySelector('[data-mobile-more-close]');
      if (mobileMoreClose) mobileMoreClose.addEventListener('click', closeMobileMore);
      document.addEventListener('click', function (e) {
        if (!mobileMorePanel.hidden && !mobileMorePanel.contains(e.target)) closeMobileMore();
      });
    }
    ...
    el('btn-inpatch-undo').addEventListener('click', function () {
      if (Store.undoArea('inPatch')) { SP.renderInputPatchGrid(); SP.renderMixerDiagram(el('mixer-diagram')); }
    });
    el('btn-inpatch-redo').addEventListener('click', function () {
      if (Store.redoArea('inPatch')) { SP.renderInputPatchGrid(); SP.renderMixerDiagram(el('mixer-diagram')); }
    });
    el('btn-inpatch-png').addEventListener('click', function () { SP.exportGridPNG('in', SP.exportFilename('输入路由矩阵', 'png')); });
    el('btn-route-undo').addEventListener('click', function () {
      if (Store.undoArea('routeGrid')) { SP.renderRouteGrid(); SP.renderMixerDiagram(el('mixer-diagram')); SP.renderMixerTable(); }
    });
    el('btn-route-redo').addEventListener('click', function () {
      if (Store.redoArea('routeGrid')) { SP.renderRouteGrid(); SP.renderMixerDiagram(el('mixer-diagram')); SP.renderMixerTable(); }
    });
    el('btn-route-png').addEventListener('click', function () { SP.exportGridPNG('route', SP.exportFilename('发送路由矩阵', 'png')); });
    el('btn-outpatch-undo').addEventListener('click', function () {
      if (Store.undoArea('outPatch')) { SP.renderOutputPatchGrid(); SP.renderMixerDiagram(el('mixer-diagram')); }
    });
    el('btn-outpatch-redo').addEventListener('click', function () {
      if (Store.redoArea('outPatch')) { SP.renderOutputPatchGrid(); SP.renderMixerDiagram(el('mixer-diagram')); }
    });
    el('btn-outpatch-png').addEventListener('click', function () { SP.exportGridPNG('out', SP.exportFilename('输出路由矩阵', 'png')); });
    ...
    function importState(data) {
      if (data.__images) {
        ...
        function restorePerformerImages(p) {
          if (p.photoId && map[p.photoId]) p.photoId = map[p.photoId];
          (p.photoGallery || []).forEach(function (photo) {
            if (photo.imageId && map[photo.imageId]) photo.imageId = map[photo.imageId];
          });
        }
        (data.performerTemplates || []).forEach(restorePerformerImages);
        (data.showRoster || []).forEach(restorePerformerImages);
        (data.standbyRosters || []).forEach(function (roster) { (roster.members || []).forEach(restorePerformerImages); });
        if (data.showPlan && data.showPlan.previewImgId && map[data.showPlan.previewImgId]) {
          data.showPlan.previewImgId = map[data.showPlan.previewImgId];
        }
        if (data.mixerPlanning) {
          ['inputPanelImgId', 'outputPanelImgId'].forEach(function (k) {
            if (data.mixerPlanning[k] && map[data.mixerPlanning[k]]) data.mixerPlanning[k] = map[data.mixerPlanning[k]];
          });
        }
